...
**Goal:** Redesign `/dashboard/profile` into a functional editor — name, avatar, and password editable via focused modals — with email verification and Stripe connect status surfaced as read-only rows.

**Architecture:** Two new API endpoints (`PATCH /api/users/me`, `POST /api/users/me/password`) plus two new React Query mutations in `useAuth.ts`. A shared `Modal` wrapper (portal + framer-motion) serves all three editor modals. The profile page itself is a full visual redesign with a hero card, account details section, and security section.

**Tech Stack:** Next.js App Router, React Query (`useMutation`/`useQuery`), bcryptjs, framer-motion, next-cloudinary (`CldImage`), lucide-react, Tailwind 4, Supabase (via `supabaseAdmin`).
...
| `app/api/users/me/password/route.ts` | Create — password change endpoint |
| `lib/hooks/useAuth.ts` | Add `useUpdateProfile`, `useChangePassword` mutations |
| `components/ui/Modal.tsx` | Create — shared modal wrapper |
| `components/ui/index.ts` | Export `Modal` |
| `app/dashboard/profile/page.tsx` | Full redesign |

---

## Task 1: PATCH /api/users/me
...
Replace the entire content of `app/api/users/me/route.ts` with:
...
Expected: same pre-existing errors in `app/api/seller/earnings/route.ts` only — no new errors.

- [ ] **Step 3: Commit**

```bash
git add app/api/users/me/route.ts
git commit -m "feat: add PATCH /api/users/me for name and avatar updates"
```

---

## Task 2: POST /api/users/me/password
...
```bash
git add app/api/users/me/password/route.ts
git commit -m "feat: add POST /api/users/me/password for password change"
```

---

## Task 3: Add useUpdateProfile and useChangePassword hooks

**Files:**
- Modify: `lib/hooks/useAuth.ts`

- [ ] **Step 1: Append two mutations to the bottom of `lib/hooks/useAuth.ts`**

Add after the closing brace of `useMe`:
...
## Task 4: Modal wrapper component
...
- [ ] **Step 2: Export Modal from `components/ui/index.ts`**
...
```bash
git add components/ui/Modal.tsx components/ui/index.ts
git commit -m "feat: add shared Modal portal component"
```
...
This task replaces the entire file. The page has four inner components:
- `HeroCard` — avatar, name, email+badge, account type, member since
- `AccountSection` — email verification row, Stripe connect row
- `SecuritySection` — change password row
- Three modal forms: `NameForm`, `AvatarForm`, `PasswordForm`

- [ ] **Step 1: Replace `app/dashboard/profile/page.tsx` with the full redesign**
...
function AvatarForm({
  ...
  const displaySrc = preview ?? undefined
  const displayAvatarUrl = preview ? null : currentAvatarUrl
      ...
      <div className="flex flex-col items-center gap-4">
        <div className="relative">
          {preview ? (
            ...
            <Avatar avatarUrl={displayAvatarUrl} name={currentName} size={96} />
          )}
        </div>
        <button
          type="button"
...
Expected: no new errors beyond the pre-existing ones in `app/api/seller/earnings/route.ts`.

- [ ] **Step 3: Commit**

```bash
git add app/dashboard/profile/page.tsx
git commit -m "feat: redesign profile page with name/avatar/password editors"
```
...
After all tasks committed, the profile page at `/dashboard/profile` will:
- Display a hero card with editable name and avatar (with hover-reveal camera button)
- Show email with inline verified/unverified badge + resend option if unverified
- Show Stripe connect status with a link to `/dashboard/billing`
- Allow password changes via a 3-field modal with show/hide toggles
- Use a shared `Modal` portal that dismisses on Escape and backdrop click
